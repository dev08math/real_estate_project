...
export default function SearchBar({propList, setpropList}) {
    const [propSet, setpropSet] = useState(() => new Set());
    const setSize = useRef(0)

    const addItem = (item) => {
        setpropSet((prev) => new Set(prev).add(item));
      };
    ...
    function onAddtion(value) {
        // you add a request here
        alert("You added a new chip");
        console.log(value);
        // makeshift addition
        addItem(value[value.length - 1]);
        for (var i = 0; i < products.length; i++) {
          if (products[i].title === value[value.length - 1]) {
            setpropList([...propList, products[i]]);
            break;
          }
        }
        setSize.current = setSize.current + 1;
        console.log(propList, "and ", setSize);
      }
    ...
    const onDeletion = (value) => {
        ...
        propSet.forEach((i) => {
            if (!value.includes(i)) val = i;
        });
        removeItem(val);

       setpropList(() => {
            let i = 0, newList = propList.slice();
  ...
  return (
    <div>
        <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} variant="filled" placeholder={setSize.current > 0 ? "" : "Your desired place(s) ?" }  />
            )}
          />
    </div>
  )
